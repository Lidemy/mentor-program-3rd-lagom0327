...
這裡我不太清楚 `const winner = A > B ? 'A' : 'B';` 這句要不要放在 `const B = ` 下面，因為我之前看別人寫的範例好像變數都會宣告最上方，但這裡因為如果 A 和 B 比分一樣就會跳出函式，所以我覺得不會那麼早計算 `winner` 是誰，不知道變數是統一宣告最上方，還是在需要用到前再宣告 ?
...
```
所以主要執行順序是 makeArr() $\rightarrow$ delete0 重複至沒有 0 存在 $\rightarrow$ deleteN 重複至沒有 N 的倍數存在 $\rightarrow$ checkN 檢查是否合格 不合格 $\rightarrow$ 再回到 makeArr()製造新長度不為 N 的倍數的陣列
...
```
所以新的程式執行是 makeArr() $\rightarrow$ checkLength() $\rightarrow$ deleteN() $\rightarrow$ checkN() $\rightarrow$ longArr() $\rightarrow$ checkLength() $\rightarrow$ ......
